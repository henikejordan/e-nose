 ...
 * @author Henike
 */
public class DAOPoeira10 extends DAO {

    public DAOPoeira10(String data_hora_ini, String data_hora_fim) {
        super("poeira_10", data_hora_ini, data_hora_fim);
    }

    @Override
    public synchronized List<Double> getValues(String info, Estatistica estatistica) {
        List<Double> data = new ArrayList<>();
        ResultSet resultado = getConecta().executaSQL("select * from poeira_10 "
        ...
        try {
            while (resultado.next()) {
                data.add(estatistica.calcula(resultado.getDouble("Poeira_10")));
            }
        } catch (Exception ex) {
    ...
    public synchronized void setValues(String data_hora, double[] valor, Estatistica estatistica, String classe) {
        try {
            ...
            PreparedStatement pst = getConecta().getConnection().prepareStatement("insert into poeira_10 "
                    + "(data_hora, poeira_10, classe) "
                    + "values(?,?,?)");
