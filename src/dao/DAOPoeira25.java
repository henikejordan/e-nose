 ...
 * @author Henike
 */
public class DAOPoeira25 extends DAO {

    public DAOPoeira25(String data_hora_ini, String data_hora_fim) {
        super("poeira_25", data_hora_ini, data_hora_fim);
    }
    ...
    public synchronized List<Double> getValues(String info, Estatistica estatistica) {
        ...
        try {
            while (resultado.next()) {
                data.add(estatistica.calcula(resultado.getDouble("Poeira_25")));
            }
        } catch (Exception ex) {
    ...
    public synchronized void setValues(String data_hora, double[] valor, Estatistica estatistica, String classe) {
        try {
            ...
            PreparedStatement pst = getConecta().getConnection().prepareStatement("insert into poeira_25 "
                    + "(data_hora, poeira_25, classe) "
                    + "values(?,?,?)");
